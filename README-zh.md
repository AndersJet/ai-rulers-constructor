...
完成上述初始化后，你可以使用 [`ruler`](https://github.com/intellectronica/ruler) CLI 工具将 rulers 部署到 AI 编码助手，获得更紧密的集成体验。

**前置条件：**
- 已安装 Node.js
- 已安装 `ruler` CLI：`npm install -g @intellectronica/ruler`

**步骤：**

1. **完成常规初始化**（方式一或方式二）。

2. **初始化 ruler：** 在目标工程根目录执行：
   ```bash
   ruler init
   ```
   这会创建 `.ruler/` 目录。

3. **复制 AGENTS.md：** 将初始化生成的 `AGENTS.md` 复制到 `.ruler/`：
   ```bash
   cp AGENTS.md .ruler/AGENTS.md
   ```

4. **部署规则：** 根据你当前使用的开发环境执行对应命令：
   ```bash
   # Claude Code
...
> [!NOTE]
> `ruler` 工具是可选的辅助工具。如果你愿意，也可以继续直接通过 AGENTS.md 使用 rulers 模板，无需安装 `ruler` CLI。
